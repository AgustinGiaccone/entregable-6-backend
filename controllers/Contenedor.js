...
//CRUD
    ...
    async save(title, price, thumbnail) {
        try {
            const content = JSON.parse(await fs.readFile(`./productos.txt`,'utf-8'))
            let newId;
            if(content.length == 0){
                newId = 1;
            }else {
            ...
            content.push(newObj);
            await fs.writeFile(`./productos.txt`,JSON.stringify(content, null, 2))
            return(newObj)
        } catch (error) {
            return(error)
        }
    ...
    async getAll() {
        try {
            const content = JSON.parse(await fs.readFile(`./productos.txt`,'utf-8')) // que paso con this.route
            return content
        } catch (error) {
            return []
        }
    ...
    async deleteAll(){
        try {
            await fs.writeFile(`./$productos.txt`,JSON.stringify([], null, 2))
            const content = JSON.parse(await fs.readFile(`./productos.txt`,'utf-8'))
            console.log(content)
        } catch (error) {
            console.log(error)
    ...
    async deleteById (id) {
        try {
            ...
            if(elementosFiltrados.length === (content.length)){
                ...
                await fs.writeFile(`./productos.txt`,JSON.stringify(elementosFiltrados, null, 2))
                return(elementosFiltrados)
            }
        } catch (error) {
            return(error)
